...
public class SysMenuServiceImpl implements SysMenuService {
    ...
    public List<SysMenuVo> selectMenuList(SysMenuBo menu, Long userId) {
        ...
        if (!LoginHelper.isSuperAdmin(userId)) {
            ...
            wrapper.inSql(SysMenu::getMenuId, sysMenuMapper.buildMenuByUserSql(userId));
        }
        ...
        if (LoginHelper.isSuperAdmin(userId)) {
            ...
            menus = sysMenuMapper.selectList(
                wrapper.in(SysMenu::getMenuType, SystemConstants.TYPE_DIR, SystemConstants.TYPE_MENU)
                    ...
                    .inSql(SysMenu::getMenuId, sysMenuMapper.buildMenuByUserSql(userId))
                    .orderByAsc(SysMenu::getParentId)
                    .orderByAsc(SysMenu::getOrderNum));
        }
    ...
    public List<Long> selectMenuListByPackageId(Long packageId) {
        ...
        if (tenantPackage.getMenuCheckStrictly()) {
            parentIds = sysMenuMapper.selectObjs(new LambdaQueryWrapper<SysMenu>()
                .select(SysMenu::getParentId)
                .in(SysMenu::getMenuId, menuIds), x -> {
                return Convert.toLong(x);
            });
        }
        return sysMenuMapper.selectObjs(new LambdaQueryWrapper<SysMenu>()
